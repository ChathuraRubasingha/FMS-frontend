...
const PendingBookingRequests_Table = () => {
  const [PendingBookingRequests_Table_List, setPendingBookingRequests_Table_List] = useState([])
  ...
  const deletePendingBookingRequests_Table = (id) => {
    alert('Are you sure to delete this record!')
    axios.delete(`http://localhost:5000/deletedriver/${id}`).then((response) => {
      setPendingBookingRequests_Table_List(
        PendingBookingRequests_Table_List.filter((items) => {
          return items.PendingBookingRequests_Table_ID != id
        }),
      )
    })
  }
  ...
  const getProductData = async () => {
    try {
      const res = await fetch(`http://localhost:5000/api/getPendingBookingRequest`)
    ...
    <div>
      <CCardHeader>
        <CCardBody>
          <CRow>
            <CCol xs={7}>
              <h5>Pending Booking Requests Registry</h5>
            </CCol>
            <CCol xs={3} sm={4} lg={5}>
...
export default PendingBookingRequests_Table
