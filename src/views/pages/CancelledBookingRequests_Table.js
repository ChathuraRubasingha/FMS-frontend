...
const CancelledBookingRequests_Table = () => {
  const [CancelledBookingRequests_Table_List, setCancelledBookingRequests_Table_List] = useState([])
  ...
  const deleteCancelledBookingRequests_Table = (id) => {
    alert('Are you sure to delete this record!')
    axios.delete(`http://localhost:5000/deletedriver/${id}`).then((response) => {
      setCancelledBookingRequests_Table_List(
        CancelledBookingRequests_Table_List.filter((items) => {
          return items.CancelledBookingRequests_Table_ID != id
        }),
      )
    })
  }
    ...
    <div>
      <CCardHeader>
        <CCardBody>
          <CRow>
            <CCol xs={7}>
              <h5>Rejected Vehicle Booking Requests Registry</h5>
            </CCol>
            <CCol xs={3} sm={4} lg={5}>
...
export default CancelledBookingRequests_Table
