...
const UnassignDriver_Table = () => {
  const [UnassignDriver_Table_List, setUnassignDriver_Table_List] = useState([])
  ...
  const deleteUnassignDriver_Table = (id) => {
    alert('Are you sure to delete this record!')
    axios.delete(`http://localhost:5000/deletedriver/${id}`).then((response) => {
      setUnassignDriver_Table_List(
        UnassignDriver_Table_List.filter((items) => {
          return items.UnassignDriver_Table_ID != id
        }),
      )
    })
  }
  ...
  const getProductData = async () => {
    try {
      const res = await fetch(`http://localhost:5000/api/getdriverunassignedvehicles`)
                      ...
                      .map((item) => {
                        return (
                          <CTableRow key={item.id}>
                            <CTableDataCell scope="row">{item.Location_ID}</CTableDataCell>
...
export default UnassignDriver_Table
