...
const UnassignVehicles_Table = () => {
  const [UnassignVehicles_Table_List, setUnassignVehicles_Table_List] = useState([])
  ...
  const deleteUnassignVehicles_Table = (id) => {
    alert('Are you sure to delete this record!')
    axios.delete(`http://localhost:5000/deletedriver/${id}`).then((response) => {
      setUnassignVehicles_Table_List(
        UnassignVehicles_Table_List.filter((items) => {
          return items.UnassignVehicles_Table_ID != id
        }),
      )
    })
  }
    ...
    <div>
      <CCardHeader>
        <CCardBody>
          <CRow>
            <CCol xs={4} lg={6}>
              <h5>Select Vehicle to Assign for Location</h5>
            </CCol>
            <CCol xs={4} sm={4} lg={6}>
...
export default UnassignVehicles_Table
