...
const CompletedBookingRequests_Table = () => {
  const [CompletedBookingRequests_Table_List, setCompletedBookingRequests_Table_List] = useState([])
  ...
  const deleteCompletedBookingRequests_Table = (id) => {
    alert('Are you sure to delete this record!')
    axios.delete(`http://localhost:5000/deletedriver/${id}`).then((response) => {
      setCompletedBookingRequests_Table_List(
        CompletedBookingRequests_Table_List.filter((items) => {
          return items.CompletedBookingRequests_Table_ID != id
        }),
      )
    })
  }
  ...
  const getProductData = async () => {
    try {
      const res = await fetch(`http://localhost:5000/api/getCompletedBookingRequest`)
    ...
    <div>
      <CCardHeader>
        <CCardBody>
          <CRow>
            <CCol xs={7}>
              <h5>Completed Vehicle Booking Requests Registry</h5>
            </CCol>
            <CCol xs={3} sm={4} lg={5}>
...
export default CompletedBookingRequests_Table
